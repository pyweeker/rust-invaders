...
fn player_movement(
	...
	//if let Ok((speed, mut transform)) = query.single_mut() {        Bevy 0.6  expected tuple, found enum `Result`     https://bevyengine.org/learn/book/migration-guides/0.5-0.6/
	...
	if let Ok((player_tf, mut ready_fire)) = query.get_single_mut() {
		if ready_fire.0 && kb.pressed(KeyCode::Space) {
			...
			let mut spawn_lasers = |x_offset: f32| {
				commands
					.spawn_bundle(SpriteBundle {
						//material: materials.player_laser.clone(),      bevy::prelude::SpriteBundle` does not have this field    https://bevyengine.org/learn/book/migration-guides/0.5-0.6/
